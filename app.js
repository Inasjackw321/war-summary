...
function showSourceWarningPopup(ch, url) {
  ...
  const cancelBtn = el.querySelector(".source-warn-cancel");
  if (cancelBtn) cancelBtn.addEventListener("click", e => { e.stopPropagation(); el.remove(); });
  el.addEventListener("click", e => { if (e.target === el) { e.stopPropagation(); el.remove(); } });
}
...
// ── Graph lightbox ────────────────────────────────────────────────────────────
// Pre-rendered PNGs for the two main charts (generated by GitHub Action)
const GRAPH_PNGS = {
  "me-chart-alerts":   { url: "data/middle_east_graph.png", label: "Israel — Red Alerts — Last 24 Hours" },
  "ua-chart-7day":     { url: "data/ukraine_graph.png",     label: "Ukraine — Missiles & Drones — Last 7 Days" },
};

function openGraphModal(imgUrl, title, isDataUrl) {
  const old = document.getElementById("graphModal");
  if (old) old.remove();

  const ts = isDataUrl ? "" : "?t=" + Date.now();
  ...
  modal.innerHTML = `
    <div class="graph-modal-inner">
      <div class="graph-modal-header">
        <span class="graph-modal-title">${title}</span>
        <div class="graph-modal-actions">
          <a class="graph-modal-download" href="${imgUrl}${ts}" download="${title.replace(/[^a-z0-9]+/gi,"-").toLowerCase()}.png">
  ...
  modal.querySelector(".graph-modal-close").addEventListener("click", close);
  document.addEventListener("keydown", function h(e) {
    if (e.key === "Escape") { close(); document.removeEventListener("keydown", h); }
  });
}
...
function initChartExpanders() {
  const EXPAND_SVG = `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>`;

  document.querySelectorAll(".chart-card").forEach(card => {
    const titleEl = card.querySelector(".sidebar-card-title");
    if (!titleEl || titleEl.querySelector(".chart-expand-btn")) return;
    ...
    btn.setAttribute("aria-label", "View chart full size");
    btn.innerHTML = EXPAND_SVG;
    titleEl.appendChild(btn);

    btn.addEventListener("click", e => {
      e.stopPropagation();
      const titleText = (titleEl.textContent || titleEl.innerText || "").trim().replace(/\s+/g, " ");

      // If this canvas has a pre-rendered high-quality PNG, prefer that
      const png = GRAPH_PNGS[canvas.id];
      if (png) {
        openGraphModal(png.url, png.label, false);
      } else {
        // Capture Chart.js canvas as PNG data URL
        try {
          const dataUrl = canvas.toDataURL("image/png");
          openGraphModal(dataUrl, titleText, true);
        } catch (err) {
          showToast("Could not capture chart", 1800);
        }
      }
    });
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Chart expanders run after panels are populated (charts exist in DOM at load)
  setTimeout(initChartExpanders, 800);
});
